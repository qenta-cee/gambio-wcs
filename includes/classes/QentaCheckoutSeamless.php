...
define('TABLE_PAYMENT_QCS', 'payment_qenta_checkout_seamless');
...
class QentaCheckoutSeamless_ORIGIN
{
	...
	/** @var GMQentaCheckoutSeamless_ORIGIN */
	protected $_seamless;


	public function __construct()
	{
		$this->_seamless = MainFactory::create_object('GMQentaCheckoutSeamless');

		$gpt        = str_replace('-', '', strtolower($this->_paymenttype));
		$this->code = sprintf('qcs_%s', $gpt);

		$c = strtoupper($this->code);

		$this->title = $this->constant("MODULE_PAYMENT_{$c}_TEXT_TITLE");

		$logoTag = ($this->_logoFilename) ? '<img src="' . DIR_WS_CATALOG . 'images/icons/qcs/' . $this->_logoFilename
		                                    . '" alt="'
	...
	public function payment_action()
	{
		...
		$q = sprintf("INSERT INTO %s (orders_id, paymentstate, paymenttype, request, created) VALUES (%d, '%s', '%s', '%s', NOW())",
		             TABLE_PAYMENT_QCS, $orders_id, 'INIT', $this->_paymenttype,
		             xtc_db_input(json_encode($init->getRequestData())));
		xtc_db_query($q);

        unset($_SESSION['qcs_consumer_device_id']);

        if($this->_paymenttype == QentaCEE\Stdlib\PaymentTypeAbstract::SOFORTUEBERWEISUNG)
            header("Location: " . $initResponse->getRedirectUrl());
        else
            require 'checkout_qenta_checkout_seamless.php';
	...
	public function selection()
	{
		...
		if(self::$dataStore === null)
		{
			...
			if(self::$dataStore !== null)
			{
				...
				$_SESSION['qcs_storage_id'] = self::$dataStore->getStorageId();
			}
			else
			{
				return false;
	...
	protected function initDataStorage()
	{
		...
		catch(\Exception $e)
		{
			...
			if(!isset($_SESSION['qcs_error_message']))
			{
				if($e instanceof GMQentaCheckoutSeamlessUserException)
				{
					$_SESSION['qcs_error_message'] = $_SESSION['gm_error_message'] = $e->getMessage();
				}
				else
				{
					$_SESSION['qcs_error_message'] = $_SESSION['gm_error_message'] = $this->_seamless->getText('datastorage_initerror');
				}
			...
			{
				unset($_SESSION['qcs_error_message']);
	...
	public function install()
	{
		...
		$q = "CREATE TABLE IF NOT EXISTS " . TABLE_PAYMENT_QCS . "
 ...
 * @return string
 */
function qcs_cfg_pull_down_invoice_provider($p_provider_id, $p_key = '')
{
 ...
 * @return string
 */
function qcs_cfg_invoice_checkbox()
{
    $name = 'MODULE_PAYMENT_QCS_INVOICE_EQUAL_ADDRESS';
 ...
 * @return string
 */
function qcs_cfg_installment_checkbox()
{
    $name = 'MODULE_PAYMENT_QCS_INSTALLMENT_EQUAL_ADDRESS';
    $checked = (defined($name)) ? (constant($name) === 'on') ? 'on' : 'off' : 'off';
    $name = 'configuration[' . $name . ']';
 ...
 * @return string
 */
function qcs_cfg_invoice_terms_checkbox()
{
    $name = 'MODULE_PAYMENT_QCS_INVOICE_PAYOLUTION_CONSENT';
    $checked = (defined($name)) ? (constant($name) === 'on') ? 'on' : 'off' : 'off';
    $name = 'configuration[' . $name . ']';
 ...
 * @return string
 */
function qcs_cfg_installment_terms_checkbox()
{
    $name = 'MODULE_PAYMENT_QCS_INSTALLMENT_PAYOLUTION_CONSENT';
    $checked = (defined($name)) ? (constant($name) === 'on') ? 'on' : 'off' : 'off';
    $name = 'configuration[' . $name . ']';
 ...
 * @return string
 */
function qcs_cfg_pull_down_installment_provider($p_provider_id, $p_key = '')
{
    $name = (($p_key) ? 'configuration[' . $p_key . ']' : 'configuration_value');
 ...
 * @return string
 */
function qcs_cfg_pull_down_customergroups($p_group_id, $p_key = '')
{
	$name = (($p_key) ? 'configuration[' . $p_key . ']' : 'configuration_value');
...
MainFactory::load_origin_class('QentaCheckoutSeamless');
