...
require_once DIR_FS_DOCUMENT_ROOT . '/ext/qenta/autoload.php';

class GMQentaCheckoutSeamless_ORIGIN
{
	/**
	 * @var string
	 */
	protected $_plugintype = 'gambio_qcs';
	...
	public $_config = array(
		...
		'options'  => array(
			'service_url'          => array(
				...
				'validator' => 'url',
				'width'     => '270px',
				'docref'    => 'https://guides.qenta.com/request_parameters#serviceurl',
				'trim'      => true
			),
			'shop_name'            => array(
				'type'     => 'text',
				'required' => false,
				'docref'   => 'https://guides.qenta.com/request_parameters#customerstatement'
			),
			...
			'send_confirm_email'   => array(
				'type'     => 'checkbox',
				'required' => false,
				'docref'   => 'https://guides.qenta.com/request_parameters#confirmmail'
			),
			'auto_deposit'         => array(
				'type'     => 'checkbox',
				'required' => false,
				'docref'   => 'https://guides.qenta.com/request_parameters#autodeposit'
			),
			'send_basket'          => array(
				'type'     => 'checkbox',
				'required' => false,
				'docref'   => 'https://guides.qenta.com/request_parameters#shopping_basket_data'
			),
			'delete_cancel'  => array(
				'type'     => 'checkbox',
				'required' => false,
		...
		'ccard'    => array(
			'pci3_dss_saq_a_enable'      => array(
				'type'     => 'checkbox',
				'required' => false,
				'docref'   => 'https://guides.qenta.com/wcs:pci3_fallback'
			),
	...
	public function __construct()
	{
		...
		$this->_txt = MainFactory::create_object('LanguageTextManager',
			array('qenta_checkout_seamless', $_SESSION['languages_id']));
	}
	...
	public function log($message, $loglevel = LOG_INFO)
	{
		...
		{
		case LOG_ERR:
			$this->_logger->error($message, 'payment', 'payment.qcs');
			break;
		case LOG_WARNING:
			$this->_logger->warning($message, 'payment', 'payment.qcs');
			break;
		case LOG_INFO:
		case LOG_DEBUG:
		default:
			$this->_logger->notice($message, 'payment', 'payment.qcs');
			break;
		}
	}
	 ...
	 *
	 * @return bool
	 * @throws \GMQentaCheckoutSeamlessException
	 */
	public function validateConfigValue($p_key, $p_value, $p_configtype = 'prod')
	{
		...
		if (!is_array($cfg))
		{
			throw new GMQentaCheckoutSeamlessException('unknown parameter');
		}
		...
		if ($cfg['required'] && !strlen($p_value))
		{
			throw new GMQentaCheckoutSeamlessException(sprintf($this->getText('param_required'),
				$this->getText($p_key)));
		}
		...
		if (array_key_exists('validator', $cfg) && $cfg['validator'])
		{
			...
			case 'url':
				...
				{
					throw new GMQentaCheckoutSeamlessException(sprintf($this->getText('param_invalid'),
						$this->getText($p_key)));
				}
				break;
			}
	...
	{
		$dataStorageInit = new QentaCEE\QMore\DataStorageClient($this->_getConfigArray());
		$dataStorageInit->setReturnUrl(xtc_href_link('callback/qenta/checkout_seamless_datastorage_return.php', '',
			'SSL'));
		...
		try
		{
			...
			if ($response->getStatus() != QentaCEE\QMore\DataStorage\Response\Initiation::STATE_SUCCESS)
			{
				$msg = array();
				...
				throw new GMQentaCheckoutSeamlessException(implode('<br/>', $msg));
			}
		} catch (Exception $e)
		{
			throw new GMQentaCheckoutSeamlessException($e->getMessage());
		}
	...
	/**
	 * init qenta datastorage
	 * @return null|\QentaCEE\QMore\DataStorage\Response\Initiation
	 * @throws \GMQentaCheckoutSeamlessException
	 */
	public function initDataStorage()
	{
		...
		try
		{
			...
			if ($response->getStatus() != QentaCEE\QMore\DataStorage\Response\Initiation::STATE_SUCCESS)
			{
				...
				throw new GMQentaCheckoutSeamlessException($msg);
			}
		} catch (Exception $e)
		{
	...
	{
		$dataStorageRead = new QentaCEE\QMore\DataStorage\Request\Read($this->_getConfigArray());
	 ...
	 *
	 * @return \QentaCEE\QMore\FrontendClient|\QentaCEE\QMore\Response\Initiation
	 * @throws \Exception
	 */
	public function initPayment($order, $paymentType)
	{
		...
		$init = new QentaCEE\QMore\FrontendClient($this->_getConfigArray());
		...
		$init->setStorageReference(session_id(), $_SESSION['qcs_storage_id']);

		if ($paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::EPS
			|| $paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::IDL
			|| $paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::TRUSTPAY
		)
		{
			$init->setFinancialInstitution($_SESSION['qcs_financialinstitution']);
		}
		...
		$returnUrl = xtc_href_link('callback/qenta/checkout_seamless_return.php',
			'fboOID=' . $order->info['orders_id'], 'SSL', false);


        $confirmUrl = xtc_href_link('callback/qenta/checkout_seamless_confirm.php', '', 'SSL', false);
        ...
        if (isset($_SESSION['qcs_consumer_device_id'])) {
            $init->consumerDeviceId = $_SESSION['qcs_consumer_device_id'];
        }

		$init->orders_id = $order->info['orders_id'];

		if ($this->getConfigValue('send_basket')
			|| ($paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::INVOICE
				&& (MODULE_PAYMENT_QCS_INVOICE_PROVIDER == 'RatePay' || MODULE_PAYMENT_QCS_INVOICE_PROVIDER == 'Wirecard'))
			|| ($paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::INSTALLMENT
				&& MODULE_PAYMENT_QCS_INSTALLMENT_PROVIDER == 'RatePay')
		)
		{
			...
			foreach ($order->products as $idx => $p)
			{
                ...
                $item = new QentaCEE\Stdlib\Basket\Item($product_id);
                $item->setName($p['name'])
	...
	public function handleConfirm($postdata)
	{
		...
		$return = QentaCEE\QMore\ReturnFactory::getInstance($postdata, $this->getConfigValue('secret'));

		if (!$return->validate())
		{
			$this->log(__METHOD__ . ':Validation error: invalid response', LOG_WARNING);

			return QentaCEE\QMore\ReturnFactory::generateConfirmResponseString('Validation error: invalid response');
		}
		...
		try
		{
			...
			xtc_db_query(sprintf("UPDATE %s SET response='%s', paymentstate='%s', modified = NOW() WHERE orders_id = %d",
				TABLE_PAYMENT_QCS, xtc_db_input(json_encode($postdata)), $return->getPaymentState(),
				$orders_id));
			...
			case QentaCEE\QMore\ReturnFactory::STATE_FAILURE:
				...
				xtc_db_query(sprintf("UPDATE %s SET message='%s' WHERE orders_id = %d", TABLE_PAYMENT_QCS,
					xtc_db_input($msg), $orders_id));
			default:
			}
			if ($ordernumber !== null)
			{
				xtc_db_query(sprintf("UPDATE %s SET ordernumber='%s' WHERE orders_id = %d", TABLE_PAYMENT_QCS,
		...
		} catch (Exception $e)
		{
			...
			return QentaCEE\QMore\ReturnFactory::generateConfirmResponseString($e->getMessage());
		}

		return QentaCEE\QMore\ReturnFactory::generateConfirmResponseString();
	}
	...
	{
		$cl = new QentaCEE\QMore\BackendClient($this->_getBackendConfigArray());
	...
	{
		$client = new QentaCEE\QMore\BackendClient($this->_getBackendConfigArray());
	 ...
	 * @param $p_currency
	 *
	 * @return \QentaCEE\QMore\Response\Backend\Deposit
	 */
	 ...
	 *
	 * @return \QentaCEE\QMore\Response\Backend\DepositReversal
	 */
	 ...
	 * @param $p_ordernumber
	 *
	 * @return \QentaCEE\QMore\Response\Backend\ApproveReversal
	 */
	 ...
	 * @param $p_currency
	 *
	 * @return \QentaCEE\QMore\Response\Backend\Refund
	 */
	 ...
	 *
	 * @return \QentaCEE\QMore\Response\Backend\RefundReversal
	 */
	 ...
	 *
	 * @return \QentaCEE\QMore\Request\Backend\TransferFund
	 * @throws \QentaCEE\Stdlib\Exception\InvalidTypeException
	 */
	...
	{
		$t_query = "SELECT * FROM payment_qenta_checkout_seamless WHERE orders_id = ':orders_id'";
	...
	{
		return QentaCEE\QMore\FrontendClient::generatePluginVersion('Gambio', $this->_getGxVersion(),
	...
	public function getConsumerData($order, $paymentType)
	{
		...
		if ($this->getConfigValue('send_additional_data')
			|| ($paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::INSTALLMENT
				|| $paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::INVOICE
				|| $paymentType == QentaCEE\Stdlib\PaymentTypeAbstract::P24)
		)
		{
			...
			if ($data !== null)
			{
				...
				} elseif (isset($_SESSION['qcs_birthdate'])) {
                    $consumerData->setBirthDate(new DateTime($_SESSION['qcs_birthdate']));
                    unset($_SESSION['qcs_birthdate']);
                }
...
class GMQentaCheckoutSeamlessException extends Exception
{
}

class GMQentaCheckoutSeamlessUserException extends GMQentaCheckoutSeamlessException
{
}

MainFactory::load_origin_class('GMQentaCheckoutSeamless');
